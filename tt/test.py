...
def get_paths(test, f):
    ...
    paths["exp_out_file"] = Path("tests/", test, "expected_output_file.txt").resolve(
        strict=True
    )
...
def run_one_test(test, f=None, nvim=False):
    ...
    if filecmp.cmp(paths["exp_out_file"], paths["gen_out_file"]):
        if "max_cpu_time" in config and config["max_cpu_time"] < commands_cpu_time:
            log(
                "{} {} {}[slow]{} {}".format(
                    info, FAIL_STR, bcolors.WARNING, bcolors.ENDC, time_str
                ),
                f,
            )
            return False
...
def main():
    ...
    parser.add_argument(
        "test", nargs="?", help="run <test> only instead of running all tests"
    )
    parser.add_argument(
        "-t",
        "--time",
        ...
        type=float,
        default=[0.1],
        help="set key delay in seconds (default 0.1)",
    )
    parser.add_argument(
        "-n", "--nvim", action="store_true", help="run in neovim instead of vim"
    )
